...
export default function InboxActivity({ module, activity, startedAtRef, onComplete }) {
  ...
  const mailboxPreviewCount = messages.filter((message) => message.attachments.length || message.linkPreview).length;

  const rootClassName = cn(
    getSimulationCategoryClass(simulationCategory),
    'grid gap-4',
    isSms ? 'email-sim inbox-sim-sms sms-summary-shell' : 'email-sim inbox-sim-mail email-summary-shell'
  );
    ...
    ? [
        {
          label: 'Canal',
          value: 'SMS',
          caption: 'Lectura rapida, tono directo y senales minimas.',
        },
        {
          label: 'Revisados',
          value: `${reviewedCount}/${messages.length || 0}`,
          caption: 'Clasifica solo lo que realmente te haria frenar.',
        },
        {
          label: 'Meta',
          value: 'Criterio',
    ...
    : [
        {
          label: 'Bandeja',
          value: 'Correo',
          caption: 'Prioriza remitente, asunto y contexto.',
        },
        {
          label: 'Revisados',
          value: `${reviewedCount}/${messages.length || 0}`,
          caption: 'En correo importan tambien adjuntos y enlaces.',
        },
        {
          label: 'Meta',
          value: 'Precision',
          caption: 'No te guies solo por la apariencia del mensaje.',
        },
      ];
  ...
  const renderSmsSidebar = () => (
    <>
      <div className="sms-sidebar-head">
        <div>
          <p className="eyebrow">Bandeja movil</p>
          <strong className="mt-1 block text-base text-sd-text">Mensajes sospechosos</strong>
          <p className="mt-2 text-sm leading-6 text-sd-text-soft">Marca solo lo que de verdad te haria frenar.</p>
        </div>
  ...
  const renderEmailSidebar = () => (
    <>
      <div className="email-sidebar-head email-sidebar-head-mail">
        <div>
          <p className="eyebrow">Bandeja principal</p>
          <strong className="mt-1 block text-base text-sd-text">Correos por revisar</strong>
          <p className="mt-2 text-sm leading-6 text-sd-text-soft">
            {cleanText(activity?.intro || 'Busca remitente, asunto, enlaces y contexto antes de clasificar.')}
          </p>
        </div>
        <div className="inbox-stage-badges">
          <span className="email-sidebar-chip">Prioridad media</span>
          <span className="email-sidebar-chip">Adjuntos</span>
          <span className="email-sidebar-chip">Enlaces</span>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {messages.map((message) => {
          const reviewItem = result?.review?.find((item) => item.id === message.id);
          const status = getInboxStatus(selections[message.id], reviewItem);
          const mailCue = message.linkPreview
            ? 'Incluye enlace'
            : message.attachments.length
              ? `${message.attachments.length} adjuntos`
              : 'Texto directo';
            ...
            >
              ...
              <div className="email-thread-main">
                ...
                <div className="email-thread-footer mt-3">
                  <span className={cn('email-thread-chip', mailCue === 'Texto directo' ? 'neutral' : 'has-link')}>
                    {mailCue}
                  </span>
                  <span
                    className={cn(
  ...
  const renderSmsReader = () => (
    <>
      <div className="sms-reader-topbar sms-reader-head border-b border-sd-border pb-4">
        <div className="sms-reader-contact">
          <span className={cn('sms-thread-avatar large', selectedStatus?.tone === 'idle' ? 'safe' : selectedStatus?.tone || 'safe')}>
      ...
      <div className="sms-reader-footer mt-4 rounded-[22px] border border-sd-border p-4">
        <p className="text-sm font-semibold text-sd-text">Como clasificarias este SMS?</p>
          ...
          >
            <strong className="block">
              {selectedReview.status === 'correct'
                ? 'Bien clasificado'
                : selectedReview.status === 'wrong'
                  ? 'Aqui conviene corregir la decision'
  ...
  const renderEmailReader = () => (
    <>
      <div className="email-reader-head border-b border-sd-border pb-4">
        <div className="email-reader-contact">
          <span
            className={cn(
              'email-thread-avatar large',
          ...
          <div>
            <strong>{selectedMessage.displayName}</strong>
            <span>{selectedMessage.from || 'Correo sin remitente claro'}</span>
          </div>
        </div>
        <div className="email-reader-status">
          <span className="email-reader-dot" />
          {selectedStatus?.label || 'Correo abierto'}
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="email-open-actions flex flex-wrap gap-2">
          <Button variant="ghost" size="compact" type="button" onClick={() => setShowDetails((value) => !value)}>
            {showDetails ? 'Ocultar detalles' : 'Ver detalles'}
          </Button>
          <Button
      ...
      {selectedMessage.warning ? (
        <div className="email-warning-card mt-4 rounded-[20px] border border-amber-300/70 bg-amber-50/90 px-4 py-3 text-sm text-amber-800">
          {selectedMessage.warning}
        </div>
      ) : null}
      ...
      <div className="email-body-card mt-4 rounded-[24px] border border-sd-border">
        <div className="email-message-stack">
          {selectedMessage.body.map((line) => (
            <p className="email-message-bubble text-sm leading-6 text-sd-text" key={`${selectedMessage.id}-${line}`}>
              {line}
            </p>
          ))}
        </div>
        ...
        {selectedMessage.linkPreview ? (
          <div className="email-link-preview mt-4 rounded-[18px] border px-4 py-3">
            <p className="email-link-label">Enlace detectado</p>
            <strong className="block text-sm text-sd-text">{selectedMessage.linkPreview}</strong>
          </div>
        ) : null}
      </div>

      <div className="email-reader-footer mt-4 rounded-[22px] border border-sd-border p-4">
        <p className="text-sm font-semibold text-sd-text">Como clasificarias este correo?</p>
        <div className="email-classify-actions mt-3">
          {['seguro', 'estafa'].map((choice) => (
            <Button
              key={choice}
              variant={selections[selectedMessage.id] === choice ? 'primary' : 'ghost'}
        ...
        {selectedReview ? (
          <div
            className={cn(
              'email-review-card mt-4 rounded-[20px] border px-4 py-4 text-sm',
              selectedReview.status === 'correct'
                ? 'border-emerald-300/80 bg-emerald-50/90 text-emerald-900'
                : selectedReview.status === 'wrong'
                  ? 'border-amber-300/80 bg-amber-50/90 text-amber-900'
      ...
      {isSms ? (
        <>
          <div className="sms-app-topbar col-span-full">
            ...
            <span>Lectura rapida y verificacion de enlace</span>
          </div>
        ...
        <div className="inbox-stage-banner is-mail col-span-full">
          <div>
          ...
          <div className="inbox-stage-badges">
            ...
            <span className="sd-badge">Enlaces</span>
            <span className="sd-badge">{`${mailboxPreviewCount} con extra`}</span>
          </div>
        </div>
      )}
      ...
      >
        ...
        {selectedMessage ? (
          <ImmersivePanel className={cn('email-reader', isSms ? 'sms-reader-body' : 'email-reader-body')}>
            {isSms ? renderSmsReader() : renderEmailReader()}
          </ImmersivePanel>
        ) : null}
      </div>
